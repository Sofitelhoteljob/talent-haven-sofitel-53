      ...
      images.forEach((_, index) => handleImageVisible(index));
        ...
        <CarouselContent className="-ml-2 md:-ml-4">
          {images.map((image, index) => <CarouselItem key={index} className="pl-2 md:pl-4 md:basis-1/2 lg:basis-1/3">
              <div className="relative aspect-[4/3] overflow-hidden rounded-lg bg-gray-100" onMouseEnter={() => handleImageVisible(index)} ref={el => imageRefs.current[index] = el}>
                {imagesLoaded[index] ? <ImageLoader src={image.src} alt={image.alt} loading={index < 3 ? "eager" : "lazy"} containerClassName="w-full h-full" className="w-full h-full hover:scale-105 transition-transform duration-300 object-fill" /> : <div className="w-full h-full flex items-center justify-center bg-gray-100">
